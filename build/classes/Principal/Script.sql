...
SELECT * FROM estancias_exterior.familias;
Select * from estancias_exterior.familias where num_hijos>=3 AND edad_maxima<10;
Select id_casa, fecha_desde, fecha_hasta, pais from estancias_exterior.casas where fecha_desde like '2020-08-01' AND fecha_hasta like '2020-08-31'AND pais like 'Reino Unido';
Select id_familia, nombre from estancias_exterior.familias where nombre like '%y';
Select id_familia, nombre, email from estancias_exterior.familias where email like '%hotmail%';
select id_casa, calle, numero, codigo_postal, ciudad, pais, fecha_desde, fecha_hasta, tiempo_minimo, tiempo_maximo, (precio_habitacion*1.05), tipo_vivienda from estancias_exterior.casas where pais = 'Reino Unido';
select sum(numero) from estancias_exterior.casas;
select * from estancias_exterior.comentarios;

SELECT f.ciudad , p.id_comentario, p.comentario
from casas f , comentarios p
where f.id_casa = p.id_casa
and  f.pais like '%Reino%'
and p.comentario like '%limpia%';
